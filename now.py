...
class NetworkModel:
    def __init__(self):
        ...
        self.devices = {
            "Laptop": {"ip": "192.168.8.42"},
            "Phone": {"ip": "192.168.8.46"},
            "Server": {"ip": "192.168.8.50"}
        }
        self.data = {
            "ping_status": {},
            "interface_data": {},
    ...
    def _parse_proc_net_dev(self, output: str) -> Dict[str, Dict[str, int]]:
        lines = output.strip().splitlines()[2:]  # تجاهل أول سطرين (header)
        data = {}
        for line in lines:
            if ':' in line:
                ...
                data[iface.strip()] = {'rx': rx_bytes, 'tx': tx_bytes}
        return data
